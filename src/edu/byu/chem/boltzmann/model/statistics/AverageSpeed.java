...
public class AverageSpeed implements StatisticWithDistribution<Velocity> {
    ...
    public AverageSpeed(SimulationInfo simInfo, Set<ParticleType> types) {
        ...
        statisticTracker = new IndividualParticleStatisticTracker(simInfo.totalNumParticles, typesRecorded, simInfo.reactionMode, Collision.PARTICLE){
            public double updatedValue(double prevValue, double velocitySquared, double timeElapsed, double mass){
                return prevValue + Math.sqrt(velocitySquared) * timeElapsed;
            }
    ...
    public double distributionFunction(double x, ParticleType particleType) {
        ...
        double avgSpeed = averageVelocity(simInfo, singleton, useFiniteSystemCorrections);
        double speedWidth = 0.76 * avgSpeed / Math.sqrt(Formulas.predictCollisionRate(simInfo, particleType, useRealGasCorrections) * simulationTime);
    ...
    public double getDistributionWidthPrediction(Velocity unit) {
        return Units.convert(DEFAULT_UNIT, unit, 0.76 * averageVelocity(simInfo, typesRecorded, useFiniteSystemCorrections)
